...
const navigationMenuTriggerStyle = cva(
  "group inline-flex h-10 w-max items-center justify-center rounded-md bg-background px-4 py-2 text-xs text-charcoal-grey font-normal transition-colors hover:text-primary focus:text-primary focus:outline-none disabled:pointer-events-none disabled:opacity-50 data-[active]:text-primary data-[state=open]:text-primary data-[state=open]:font-bold relative"
);
...
>(({ className, children, isActive, ...props }, ref) => (
  <NavigationMenuPrimitive.Trigger
    ...
    <div
      className={cn(
        "absolute bottom-1 left-[calc(50%-1rem)] -translate-x-[calc(50%-0.5rem)] rounded-full w-6 h-1 bg-primary transition-all",
...
>(({ className, ...props }, ref) => (
  <div className={cn("absolute left-0 top-full flex justify-center")}>
    <NavigationMenuPrimitive.Viewport
      className={cn(
        "origin-top-center relative mt-1.5 h-[var(--radix-navigation-menu-viewport-height)] w-full overflow-hidden rounded-[24px] bg-background text-popover-black shadow-[3px_4px_4px_1px_#0000001A] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-90 md:w-[var(--radix-navigation-menu-viewport-width)]",
        className
      )}
      ref={ref}
      {...props}
